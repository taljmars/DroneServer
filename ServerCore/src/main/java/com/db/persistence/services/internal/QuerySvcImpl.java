...
public class QuerySvcImpl extends TokenAwareSvcImpl implements QuerySvc {
	...
	public synchronized <T extends BaseObject> List<? extends BaseObject> query(QueryRequest queryRequest)
	{
		...
		List<? extends BaseObject> lst = workSession().getQueryExecutor().createNamedQuery(queryStr, params, clz, queryRequest.getOffset(), queryRequest.getLimit());
		LOGGER.debug("Results " + lst);
		List<T> arr = new ArrayList();
